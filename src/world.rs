...
impl MapInfoList {
    pub fn new(conn: &rusqlite::Connection, user_id: isize) -> Self {
        ...
        let mut stmt = conn.prepare(sql_stmt::MAP_INFO).unwrap();
        let map_infoes = stmt
...
impl MapInfo {
    fn get_map_affinity(&mut self, conn: &rusqlite::Connection) {
        let mut characters = Vec::new();
        let mut multiplier = Vec::new();
        let mut stmt = conn.prepare(sql_stmt::MAP_AFFINITY).unwrap();
        let infoes = stmt.query_map(&[&self.map_id], |row|
            (row.get("part_id"), row.get("multiplier"))
        ).unwrap();
    ...
    fn get_rewards(&mut self, conn: &rusqlite::Connection) {
        let mut stmt = conn.prepare(sql_stmt::MAP_REWARD).unwrap();
        let rewards = stmt
...
#[get("/world/map/me")]
pub fn world_map(conn: ZrcDB) -> Json<MapInfoList> {
    Json(MapInfoList::new(&*conn, STATIC_USER_ID))
}
